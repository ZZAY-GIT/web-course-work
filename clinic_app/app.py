...
def register_route():
    ...
    try:
        ...
        new_patient = Patient(
            user_id=new_user.id, full_name=full_name, birth_date=birth_date, phone=phone
        )
...
def patient_cabinet():
    ...
    upcoming_appt = (
        ...
        .join(ScheduleSlot)
        .filter(ScheduleSlot.date >= date.today())
...
def confirm_booking():
    ...
    if not slot or slot.status != "free":
        flash(
            "Выбранный временной слот уже занят. Пожалуйста, выберите другое время.",
            "error",
        )
    ...
    if slot_datetime < datetime.now():
        flash(
            "Выбранное время приема уже прошло. Невозможно записаться в прошлое.",
            "error",
        )
        return redirect(url_for("booking_page"))
    ...
    try:
        ...
        flash(
            "Вы успешно записались на приём! Подробности в Вашем личном кабинете.",
            "success",
        )
        return redirect(url_for("patient_cabinet"))
...
def download_document(doc_id):
    ...
    return send_from_directory(
        app.config["UPLOAD_FOLDER"], doc.file_path, as_attachment=as_attachment
    )
...
def doctor_cabinet():
    ...
    if view_type == "week":
        ...
        appts = (
            Appointment.query.join(ScheduleSlot)
            .filter(
        ...
        appts = (
            Appointment.query.join(ScheduleSlot)
            .filter(
                Appointment.doctor_id == doctor.id, ScheduleSlot.date == selected_date
            )
...
def save_visit():
    ...
    try:
        ...
        flash(
            f"Приём пациента {appt.patient.full_name} успешно завершен и занесен в историю.",
            "success",
        )
...
def admin_panel():
    ...
    slots = (
        ScheduleSlot.query.filter(ScheduleSlot.date >= date.today())
        .order_by(ScheduleSlot.date, ScheduleSlot.start_time)
        .all()
    )
...
def add_slot():
    ...
    try:
        ...
        end_time = (
            datetime.combine(slot_date, start_time) + timedelta(minutes=30)
        ).time()

        # Проверяем наложение расписания
        existing = ScheduleSlot.query.filter_by(
            doctor_id=int(doctor_id), date=slot_date, start_time=start_time
        ).first()

        if existing:
            flash(
                "Данный временной интервал для этого врача уже добавлен в расписание клиники.",
                "error",
            )
            return redirect(url_for("admin_panel"))
...
def admin_doctors():
    ...
    return render_template(
        "admin_doctors.html", doctors=doctors, specializations=specializations
    )
...
def admin_delete_doctor(doctor_id):
    ...
    if doc.appointments:
        flash(
            "Невозможно удалить врача, так как к нему уже записаны пациенты.", "error"
        )
        return redirect(url_for("admin_doctors"))
...
def admin_specializations():
    ...
    for spec in specializations:
        ...
        specs_with_counts.append(
            {"id": spec.id, "name": spec.name, "doc_count": doc_count}
        )

    return render_template(
        "admin_specializations.html", specializations=specs_with_counts
    )
...
def admin_delete_specialization(spec_id):
    ...
    if has_doctors:
        flash(
            f"Невозможно удалить категорию «{spec.name}», так как к ней привязаны практикующие врачи.",
            "error",
        )
        return redirect(url_for("admin_specializations"))
...
def admin_delete_service(service_id):
    ...
    if srv.appointments:
        flash(
            "Невозможно удалить услугу, так как по ней уже зарегистрированы приёмы.",
            "error",
        )
        return redirect(url_for("admin_services"))
...
def admin_cancel_appointment(appt_id):
    ...
    try:
        ...
        db.session.delete(
            appt
        )  # Удаляем саму запись, чтобы не нарушать unique constraint на slot_id!
        db.session.commit()
        flash(
            "Запись пациента успешно отменена, временной интервал освобожден.",
            "success",
        )
    except Exception as e:
        db.session.rollback()
...
def page_not_found(e):
    return render_template(
        "base.html",
        content="<div class='text-center py-5'><h1 class='display-1 text-danger font-bold'>404</h1><h3>Страница не найдена</h3><p class='text-muted'>Запрашиваемый URL-адрес отсутствует на нашем сервере.</p><a href='/' class='btn btn-primary mt-3'>На главную страницу</a></div>",
    ), 404
