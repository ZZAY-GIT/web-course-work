...
def create_seed_app():
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = (
        os.environ.get("DATABASE_URL") or "sqlite:///clinic.db"
    )
...
def seed_database():
    ...
    with app.app_context():
        ...
        try:
            ...
            print(
                f"Ошибка при проверке наличия данных (возможно, таблицы не готовы): {e}"
            )
        ...
        # Пациенты
        u_patient1 = User(
            email="patient@clinic.ru", role="patient"
        )  # Основной пациент (Амплеенков)
